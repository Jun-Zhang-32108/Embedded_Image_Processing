...
# File: run_m.sh
# Run the whold single-bare project including building the BSP project, download the hardware and software 
# This script
...
nios2-bsp hal $BSP_PATH $CORE_FILE \
	  ...
	  # The following command is problematic. If we don't command this line, we may get wrong outputs. The reason is remained unknown so far.
